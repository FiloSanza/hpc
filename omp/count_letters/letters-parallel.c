...
int make_hist( const char *text, int hist[26] )
{
    ...
    int thread_hist[num_threads][26];
    memset(thread_hist, 0, sizeof(int) * num_threads * 26);

#pragma omp parallel num_threads(num_threads) default(none) shared(num_threads, thread_hist, n, text) reduction(+:nlet)
    {
