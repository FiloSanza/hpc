...
an MPI program that computes the Mandelbrot set. This is not a truly
...
whole set). The program produces an output file `mandebrot.ppm`
containing a graphical representation of the Mandelbrot set in PPM
format (_Portable Pixmap_). If you don't have a suitable viewer, you
can convert the image, e.g., into PNG format with the command:

        convert mandelbrot.ppm mandelbrot.png

The goal of this exercise is to write a parallel version of the
provided program, where all MPI processes cooperate in the
...
More specifically, each process computes a portion of the image of
size $\mathit{xsize} \times (\mathit{ysize} / P)$. This is an
...
to form the final result. This is done using function
`MPI_Gather()`. This is a color image where three bytes are used to
encode the color of each pixel. Therefore, the `MPI_Gather()`
operation will transfer blocks of $(3 \times \mathit{xsize} \times
\mathit{ysize} / P)$ elements of type `MPI_BYTE`.
