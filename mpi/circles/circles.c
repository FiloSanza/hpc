...
input circles. The most efficient way to do so is let the master
...
int main( int argc, char* argv[] )
{
    ...
    /* Generate the points and compute how many of them lie inside the a circle */
    int my_k = K / comm_sz;
    
    /* The master handles the remaining points */
    if (my_rank == 0) {
        my_k += K % comm_sz;
    }

    c = inside(x, y, r, N, my_k);
