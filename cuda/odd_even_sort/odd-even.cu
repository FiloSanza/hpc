...
__global__ void kernel_run_phase(int *v, bool is_odd, int n)
{
    const int thread_idx = threadIdx.x + blockDim.x * blockIdx.x;
    // each thread will deal with a pair of numbers
    const int first_idx = 2 * thread_idx + (is_odd ? 1 : 0);

    if (first_idx >= n-1) {
        return;
    }

    cmp_and_swap(&v[first_idx], &v[first_idx+1]);
}
...
void odd_even_sort( int* v, int n )
{
    ...
    for (int phase = 0; phase < n; phase++) {
        kernel_run_phase<<<BLKNUM,BLKSIZE>>>(d_v, phase % 2, n);
        cudaCheckError();
    }
